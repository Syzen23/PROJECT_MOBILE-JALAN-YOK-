...
class _PlanScreenState extends State<PlanScreen>
  ...
  Future<void> _loadDestinations() async {
    ...
    setState(() {
      destinations = data;
      if (destinations.isNotEmpty) _selectedDestination = destinations[0];
      _isLoading = false;
    });
  }
  ...
  Destination _mapApiDestination(ApiDestination destination) {
    return Destination(
      id: destination.id.toString(),
      title: destination.nama,
      location: destination.lokasi,
      image: destination.gambar,
      rating: destination.rating,
      visitors: destination.kategori,
      tiket: 10000,
      waktu: 2,
    );
  }
    ...
    try {
      ...
      try {
        ...
        final response = await http
            .get(url, headers: {'User-Agent': 'JalanYokApp/1.0'})
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            _currentLocationName = city.isNotEmpty && placeName != city
                ? '$placeName, $city'
                : placeName;
            _locationError = false;
          });
  ...
  void _hitungBudget() {
    ...
    for (var f in fuelTypes) {
      if (f['label'] == fuelType) {
        hargaBbm = f['harga'] as double;
        break;
      }
    }

    // 1. BBM
    _biaya['bbm'] = isKendaraanPribadi && konsumsiBbm > 0
        ? (jarak / konsumsiBbm) * hargaBbm
        : 0;
    // 2. Tol
    _biaya['tol'] = isKendaraanPribadi
        ? (double.tryParse(_controllers['tol']!.text) ?? 0)
        : 0;
    // 3. Transport umum
    _biaya['transportUmum'] = !isKendaraanPribadi
        ? (double.tryParse(_controllers['tiketTransport']!.text) ?? 0)
        : 0;
    // 4. Tiket masuk
    ...
    // 7. Penginapan
    final hargaMalam =
        double.tryParse(_controllers['hargaPenginapan']!.text) ?? 0;
  ...
  Future<void> _simpanKeRiwayat() async {
    ...
    if (user == null || _selectedDestination == null) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Harap login dan pilih destinasi.')),
        );
      return;
    }
                ...
                child: TabBarView(
                  ...
                  children: [
                    ...
                    BudgetResultTab(
                      ...
                      onBukaPeta: _bukaPeta,
                      onSimpan: _simpanKeRiwayat,
                    ),
                  ],
                ),
              ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Autocomplete<Destination>(
                              ...
                              fieldViewBuilder:
                                  (context, ctrl, node, onSubmit) => TextField(
                                    controller: ctrl,
                                    focusNode: node,
