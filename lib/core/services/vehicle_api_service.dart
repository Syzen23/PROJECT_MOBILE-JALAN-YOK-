...
class VehicleApiService {
  ...
  static Future<List<VehicleEntry>> getAllVehicles({
    ...
    try {
      final response = await http
          .get(
            Uri.parse('$_baseUrl/api/kendaraan'),
      ...
      if (response.statusCode != 200) {
        debugPrint(
          'VehicleApiService Error: ${response.statusCode} - ${response.body}',
        );
      ...
      debugPrint('VehicleApiService Error: $e');
      return VehicleDatabase.vehicles;
    }
  }
  ...
  static Future<List<VehicleEntry>> searchVehicle({
    ...
    return vehicles.where((vehicle) {
      final matchesKind =
          isMotorcycle == null || vehicle.isMotorcycle == isMotorcycle;
      final matchesMake = vehicle.make.toLowerCase() == make.toLowerCase();
      final matchesModel =
          query.isEmpty || vehicle.model.toLowerCase().contains(query);
